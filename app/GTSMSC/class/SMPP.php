...
class SMPP
{
    ...
    // Je remplace la valeur par de faut 0 de la variable $sms_source_addr_ton par 5
    ...
    var $sms_null_terminate_octetstrings = false;
    ...
    function bindReceiver($login, $pass){
        ...
        if($this->debug) echo "<br>Binding receiver...\n\n";
        $status=$this->_bind($login, $pass, 0x00000001);
        if($this->debug) echo "<br>Binding status  : $status\n\n";
    ...
    function bindTransmitter($login, $pass){
        if($this->state!="open")return false;
        if($this->debug) echo "<br>Binding transmitter...\n\n";
    ...
    function close(){
        ...
        if($this->debug) echo "<br>Unbinding...\n\n";
        $status=$this->sendCommand(0x00000006,"");
        if($this->debug) echo "<br>Unbind status   : $status\n\n";
        fclose($this->socket);
        $this->state="closed";
    }
    /**
    ...
    function readSMS(){
        ...
        $command_id=0x00000005; $command_id2 = 0x00000103;
        //check the queue
        for($i=0;$i<count($this->pdu_queue);$i++){
            $pdu=$this->pdu_queue[$i];
            if($pdu['id']==$command_id||$pdu['id']==$command_id2){
                //remove responce
        ...
        //read pdu
        do{
            if($this->debug) echo "<br>read sms...\n\n";
            $pdu=$this->readPDU();
        ...
        }while($pdu && $pdu['id']!=$command_id && $pdu['id']!=$command_id2);
        if($pdu){
            array_pop($this->pdu_queue);
            print_r($pdu);
            //return $this->parseSMS($pdu);
            if($this->debug) echo "<br>PDU id : ".$pdu['id'];
            if ($pdu['id']==$command_id) return $this->parseSMS($pdu);//echo " : deliver_sm";
            if ($pdu['id']==$command_id2) return $this->parse_data_sm_SMS($pdu);//echo " : data_sm";
        }
        return false;
    }
    /**
...
//		$short_message = $text;
        ...
        $this->d_smpp($status,"SMPP SEND STATUS");
        //*/
        return ($status===0);
    }
    ...
    function parse_data_sm_SMS($pdu){
        //check command id
        if($pdu['id']!=0x00000103) return false;
        //unpack PDU
        $ar=unpack("C*",$pdu['body']);
        $sms=array('service_type'=>$this->getString($ar,6),
            'source_addr_ton'=>array_shift($ar),
            'source_addr_npi'=>array_shift($ar),
            'source_addr'=>$this->getString($ar,65),
            'dest_addr_ton'=>array_shift($ar),
            'dest_addr_npi'=>array_shift($ar),
            'destination_addr'=>$this->getString($ar,65),
            'esm_class'=>array_shift($ar),
            'registered_delivery'=>array_shift($ar),
            'data_coding'=>array_shift($ar),
            'short_message'=>""
        ...
        while(count($ar)>0){
            $tag_type = (int)$this->getIntString($ar,2);
            if($this->debug) echo("<br>Tag : $tag_type");
            if ($tag_type == 436){
                if($this->debug) echo('(message)');
                $msg_length= (int)$this->getIntString($ar,2);
                if($this->debug) echo("<br> longueur: $msg_length");
                $msg_value=$this->getString($ar,$msg_length);
                $sms['short_message']=$msg_value;
            }
        }
//
//			'protocol_id'=>array_shift($ar),
...
//			'short_message'=>$this->getString($ar,255)
        ...
        //send responce of recieving sms
        $this->sendPDU(0x80000103, "\0", $pdu['sn']);
        return $sms;
    }
